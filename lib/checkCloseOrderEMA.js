...
const checekOrderEMA = async (body, apiKey, secretKey) => {
  ...
  try {
    if (data?.binanceMarket) {
      ...
      await delay(2000)
      let result = 0
      const tradesReal = await apiBinance.getPnl(
        symbol,
        data?.binanceMarket?.orderId,
        apiKey,
        secretKey
      )
      const valueReal = tradesReal[tradesReal.length - 1].income
      result = valueReal || margin
      //test//////////////////////////////////
      const deBUG = {
        symbol: symbol,
        text: 'debug',
        msg: `OrderId : ${data?.binanceMarket?.orderId} \nค่าที่ถูกต้อง PNL\n${value}`
      }
