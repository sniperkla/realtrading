...
const calTakeprofit4Step2 = async (
  symbol,
  side,
  quantity,
  type,
...
module.exports = { calTakeprofit4Step2 }
...
const LongTP4 = async (body, qtyPrecision) => {
  ...
  await takeprofitZone(priceDistanceLong, priceCal, symbol, side)
...
const ShortTP4 = async (body, qtyPrecision) => {
  ...
  const takeprofit4 = priceCal + TP4L
  const quantity4 = checkQuantity(quantity * 0.25, qtyPrecision)
  await takeprofitZone(priceDistanceShort, priceCal, symbol, side)
...
const takeprofitZone = async (priceDistance, priceCal, symbol, side) => {
  let takeprofit2 = 0
  let takeprofit3 = 0
  let takeprofit4 = 0

  const TP4 = priceDistance * 0.25
  const TP3 = priceDistance * 0.5
  const TP2 = priceDistance * 0.75

  if (side === 'SELL') {
    takeprofit4 = priceCal + TP4
    takeprofit3 = priceCal + TP3
    takeprofit2 = priceCal + TP2
  } else if (side === 'BUY') {
    takeprofit4 = priceCal - TP4
    takeprofit3 = priceCal - TP3
    takeprofit2 = priceCal - TP2
