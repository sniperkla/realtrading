...
const orderTp = async (result, side, symbol, type, apiKey, secretKey) => {
  let count = 0
  let binanceTakeProfitOrders1 = []

  const lastIndex = result.tpDetail.tp.length - 1
  ...
  for (let i = 0; i < result.tpDetail.tp.length; i++) {
    ...
    if (lastIndex === i) {
      const binanceTakeProfitPayloadCP = await apiBinance.postBinannce(
      ...
      if (binanceTakeProfitPayloadCP.status === 200) {
        ...
        const buyit = {
          text: 'takeprofit',
          symbol: symbol,
          msg: `🟢 ${symbol} ตั้ง TakeProfit loop1 # CP สำเร็จ : ${binanceTakeProfitOrders1[i].stopprice} / QTY :${binanceTakeProfitOrders1[i].qty} 🟢`
        }
        await lineNotifyPost.postLineNotify(buyit)
      } else {
        {
          const buyit = {
            text: 'takeprofit',
            symbol: symbol,
            msg: `🟢 ${symbol} ตั้ง TakeProfit loop1  # CP ไม่สำเร็จ : ${binanceTakeProfitPayloadCP?.data.msg} / QTY :${binanceTakeProfitOrders1[i].qty}  🟢`
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
      }
