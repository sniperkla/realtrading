...
const orderTp = async (
  ...
  for (let i = 0; i < result.tpDetail.tp.length; i++) {
    ...
    console.log('debug payload', binanceTakeProfitOrders1)
  ...
  for (let i = 0; i < result2.tpDetail.tp.length; i++) {
    ...
    if (
      binanceTakeProfitPayload.status === 200 &&
      !binanceTakeProfitPayloadCP
    ) {
      await updateLogTakeProfit7Step(
        symbol,
        binanceTakeProfitPayload.data,
      ...
      await lineNotifyPost.postLineNotify(buyit)
    } else if (binanceTakeProfitPayloadCP?.status === 200) {
      console.log('cpsuccess')
      await updateLogTakeProfit7Step(
        symbol,
        binanceTakeProfitPayload.data,
        `binanceTakeProfitCp`
    ...
    } else {
      {
        const buyit = {
          text: 'takeprofit',
          symbol: symbol,
          msg: `🟢 ${symbol} ตั้ง TakeProfit loop2  # ${count} ไม่สำเร็จ : ${
            binanceTakeProfitPayloadCP?.status === 400
              ? binanceTakeProfitPayloadCP?.data.msg
              : binanceTakeProfitPayload?.data.msg
          } 🟢`
        }
        await lineNotifyPost.postLineNotify(buyit)
