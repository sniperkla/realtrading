    ...
    if (binanceTakeProfitPayload.status === 200) {
      ...
      const buyit = {
        ...
        msg: `✅ ${symbol} ตั้ง TakeProfit loop1 #${i + 1} สำเร็จ : ${
          binanceTakeProfitOrders1[i].stopprice
        } / QTY : ${binanceTakeProfitOrders1[i].qty} ✅ `
      }
      await lineNotifyPost.postLineNotify(buyit)
    } else {
      const buyit = {
    ...
    if (result2.tpDetail.tp.length === i + 1) {
      ...
      if (binanceTakeProfitPayloadCP.status === 200) {
        ...
        const buyit = {
          text: 'takeprofit',
          symbol: symbol,
          msg: `✅ ${symbol} ตั้ง TakeProfit loop2 # CP สำเร็จ : ${binanceTakeProfitOrders2[i].stopprice} / QTY : ${binanceTakeProfitOrders2[i].qty} ✅`
        }
        await lineNotifyPost.postLineNotify(buyit)
      } else {
        {
          const buyit = {
            text: 'takeprofit',
            symbol: symbol,
            msg: `❌ ${symbol} ตั้ง TakeProfit loop2  # CP ไม่สำเร็จ : ${binanceTakeProfitPayloadCP?.data.msg} / QTY : ${binanceTakeProfitOrders2[i].qty} ❌`
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
      }
      ...
      if (binanceTakeProfitPayload.status === 200) {
        ...
        const buyit = {
          text: 'takeprofit',
          symbol: symbol,
          msg: `✅ ${symbol} ตั้ง TakeProfit loop2 # ${count} สำเร็จ : ${binanceTakeProfitOrders2[i].stopprice} / QTY : ${binanceTakeProfitOrders2[i].qty} ✅`
        }
        await lineNotifyPost.postLineNotify(buyit)
      } else {
        {
          const buyit = {
            text: 'takeprofit',
            symbol: symbol,
            msg: `❌ ${symbol} ตั้ง TakeProfit loop2 # ${count} ไม่สำเร็จ : ${binanceTakeProfitPayload?.data.msg} / QTY : ${binanceTakeProfitOrders2[i].qty} ❌`
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
      }
