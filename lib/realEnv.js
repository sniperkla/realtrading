...
const buyingBinance = async (body) => {
  ...
  try {
    ...
    if (binanceMarket.status === 400) {
      ...
      if (side === 'BUY' && priceCal >= body.BTU) {
        await updateLogLockStopLoss(symbol)
      } else if (side === 'SELL' && priceCal <= body.BTL) {
        await updateLogLockStopLoss(symbol)
      }
...
const updateLogLockStopLoss = async (symbol, data) => {
  const updated = await Log.updateOne(
    { symbol: symbol },
    { $set: { lockStopLoss: { lockBigTrend: true } } }
  )
}
