    ...
    if (lastIndex === i) {
      ...
      if (binanceTakeProfitPayloadCP.status === 200) {
        ...
        const buyit = {
          text: 'takeprofit',
          symbol: symbol,
          msg: `🟢 ${symbol} ตั้ง TakeProfit loop1 # CP สำเร็จ : ${binanceTakeProfitOrders1[i].stopprice}\n QTY :${binanceTakeProfitOrders1[i].qty}`
        }
        await lineNotifyPost.postLineNotify(buyit)
      } else {
        {
          const buyit = {
            text: 'takeprofit',
            symbol: symbol,
            msg: `❌ ${symbol} ตั้ง TakeProfit loop1  # CP ไม่สำเร็จ : ${binanceTakeProfitPayloadCP?.data.msg}\n QTY :${binanceTakeProfitOrders1[i].qty} `
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
      }
