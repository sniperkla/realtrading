...
const checkSideMartingale = async () => {
  try {
    const martingale = await Martingale?.find()
    const checkSideOpened = await log.find()
    let sell = []
    let buy = []
    ...
    const buyit = {
      ...
      msg: `\n จำนวนเปิดไม้ SELL : ${
        sell.length
      } มาติงเกลรวม : ${totalPreviousMarginSelled.toFixed(
        2
      )}  \n จำนวนเปิดไม้ BUY : ${
        buy.length
      } มาติงเกลรวม :  ${totalPreviousMarginBuyed.toFixed(
        2
      )} \n จำนวนเปิดไม้รวม : ${sell?.length + buy?.length}`
    }
    await lineNotifyPost.postLineNotify(buyit)
  } catch (error) {
    return error.response
