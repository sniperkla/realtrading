...
const martingale = async () => {
  ...
  for (let i = 0; i < dataMartingale.length; i++) {
    for (let j = 0; j < dataMartingaleLog.length; j++) {
      if (dataLog[i] !== dataMartingale[j]) {
        if (dataMartingale[i].previousMargin === 2560) {
          level.lv10.all = level.lv10.all + 1
        } else if (dataMartingale[i].previousMargin === 1280) {
          level.lv9.all = level.lv9.all + 1
        } else if (dataMartingale[i].previousMargin === 640) {
          level.lv8.all = level.lv8.all + 1
        } else if (dataMartingale[i].previousMargin === 320) {
          level.lv7.all = level.lv7.all + 1
        } else if (dataMartingale[i].previousMargin === 160) {
          level.lv6.all = level.lv6.all + 1
        } else if (dataMartingale[i].previousMargin === 80) {
          level.lv5.all = level.lv5.all + 1
        } else if (dataMartingale[i].previousMargin === 40) {
          level.lv4.all = level.lv4.all + 1
        } else if (dataMartingale[i].previousMargin === 20) {
          level.lv3.all = level.lv3.all + 1
        } else if (dataMartingale[i].previousMargin === 10) {
          level.lv2.all = level.lv2.all + 1
        } else if (dataMartingale[i].previousMargin === 5) {
