    ...
    for (j = 0; j < Object.keys(pnls).length; j++) {
      if (pnls[i].symbol === margins[j].symbol) {
        if (pnls[i].pnl > margins[j].margin * 1.8) {
          const check = await setting.findOne({ _id: 'takeprofitauto' })
          if (check[0].setting === true) {
            const x = await SCMP.scmpSellALL(pnls[i].symbol, apiKey, secretKey)
            await martingaleUpdate.update(pnls[i].symbol, 'LOSE', margin)
          }
          const buyit = {
            text: 'alert',
            msg: `😎 แจ้งเตือน\nเหรียญ : ${
              pnls[i].symbol
            }\nขายออโต้ที่กำไร 180% : ${parseFloat(pnls[i].pnl).toFixed(2)} 😎
        `
          }
          await lineNotifyPost.postLineNotify(buyit)
        ...
        `
          ...
          const buyit = {
            ...
            msg: `😎 แจ้งเตือน\nเหรียญ : ${
              pnls[i].symbol
            }\nได้กำไรมากกว่า 100% : ${parseFloat(pnls[i].pnl).toFixed(2)} 😎
        `
          }
          await lineNotifyPost.postLineNotify(buyit)
          ...
          const buyit = {
            ...
            msg: `😎 แจ้งเตือน\nเหรียญ : ${
              pnls[i].symbol
            }\nได้กำไรมากกว่า 60% : ${parseFloat(pnls[i].pnl).toFixed(2)} 😎
        `
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
...
const tpPayload = async (status, symbol) => {
  ...
  console.log('maxdraw down check')
  console.log('maxdraw down', maxDrawDown)
  console.log('maxdraw martingaleMonthly', martingaleMonthly)
