  ...
  if (log.length !== 0) {
    ...
    for (let i = 0; i < symbol.length; i++) {
      ...
      if (
        ...
        await Bos.deleteOne({ symbol: symbol[i] })
...
const checkBoss1min = async () => {
  const symbol = await Bos.find()

  const sym = symbol.map((item) => {
    return item.symbol
  })

  for (let i = 0; i < sym.length; i++) {
    const checkBoss = await checkBos.togleBos(sym[i])
    if (checkBoss) {
      console.log('success buying via limit jaa', sym[i])
    } else {
...
const checkOpenOrder4Bos = async () => {
  const symbol = await Log.find()
  const bos = await Bos.find()
  if (bos.length != 0) {
    const sym = symbol.map((item) => {
      return { symbol: item.symbol, orderId: item.binanceMarket.orderId }
    })
    for (let i = 0; i < sym.length; i++) {
      const checkOrder = await apiBinance.getOrder(
        sym[i].orderId,
        sym[i].symbol,
        apiKey,
        secretKey
      )
      const bosData = bos.filter((item) => {
        return item.symbol === sym[i].symbol
      })
      if (checkOrder.status === 'FILLED' && bosData[0].status === 'NEW') {
        const takeProfitMarket = await apiBinance.postBinannce(
          bosData[0].symbol,
          bosData[0].side,
          0,
          'BOS_TAKE_PROFIT_LIMIT',
          bosData[0].stopPriceCal.value,
          true,
          bosData[0].takeProfit.value,
          apiKey,
          secretKey,
          bosData[0].priceCal.value
        )

        if (takeProfitMarket.status === 200) {
          const buyit = {
            text: 'takeprofit',
            msg: `🟢 ${bosData[0].symbol} ตั้ง Takeprofit สำเร็จ\n                     ${bosData[0].takeProfit.value} 🟢`
          }
          await updateLogTakeProfit(bosData[0].symbol, takeProfitMarket.data)
          await lineNotifyPost.postLineNotify(buyit)
        } else if (takeProfitMarket.status === 400) {
          const buyit = {
            text: 'error',
            msg: takeProfitMarket.data.msg
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
        const stopLossMarket = await apiBinance.postBinannce(
          bosData[0].symbol,
          bosData[0].side,
          0,
          'BOS_STOP_LOSS_LIMIT',
        ...
        if (stopLossMarket.status === 200) {
          const buyit = {
            text: 'stoplossfirst',
            type: 'STOP_MARKET',
            msg: `🔴 ${bosData[0].symbol} ตั้ง StopLoss สำเร็จ\n                     ${bosData[0].stopPriceCal.value} 🔴`
          }
          await updateLogStopLoss(bosData[0].symbol, stopLossMarket.data)
          await lineNotifyPost.postLineNotify(buyit)
        } else if (stopLossMarket.status === 400) {
          const buyit = {
            text: 'error',
            symbol: bosData[0].symbol,
            type: 'STOP_MARKET',
            msg: stopLossMarket.data.msg
          }
          await lineNotifyPost.postLineNotify(buyit)
        }
        if (takeProfitMarket.status === 200 && stopLossMarket.status === 200) {
          await Bos.updateOne(
            { symbol: bosData[0].symbol },
            { status: 'FILLED' }
          )
        }
      } else console.log('you cant set takeprofithere')
    }
  } else console.log('no bos data')
}
