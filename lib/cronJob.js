  ...
  if (checkSetting[0]?.status === true) {
    ...
    const compare =
      ...
      checkSetting[0]._id === 'sellAll_when_toggle_Mbalnce2Level'
        ? getAccountInfo?.totalMarginBalance
        : getAccountInfo?.totalUnrealizedProfit

    console.log('unPNL balance', getAccountInfo?.totalUnrealizedProfit)
    console.log('margin balance', getAccountInfo?.totalMarginBalance)
    console.log(`checkSetting value ${checkSetting[0]._id}`)
    ...
    if (
      ...
      console.log(`should be here comaper ${compare}`)
      await setting.findOneAndUpdate(
    ...
    if (checkSetting[0]?._id === 'sellAll_when_toggle_2Level') {
      ...
      if (compare <= checkSellAll?.end && checkSellAll.executed) {
        ...
        console.log('ur here test for sell all mlv2 why you here mother fucker')
        const buyit = {
          text: 'debug',
