...
const checkStopLoss4Step = async () => {
  ...
  console.log('find25', find25)

  let alls = {}
  alls = find25.lockStopLoss.map((item) => {
    want = { zone1: item.zone1, zone2: item.zone2, zone3: item.zone3 }

    console.log('want', want)
    return want
  })

  console.log('find25', alls)

  const buyit = {
    text: 'updatestoploss',
    msg: `SUMMARY  zone1: ${alls?.zone1?.length} / zone2: ${
      alls?.zone2?.length
    } / zone3: ${alls?.zone3?.length || 'error'}`
  }
  await lineNotifyPost.postLineNotify(buyit)
