  ...
  for (i = 0; i < Object.keys(pnls).length; i++) {
    for (j = 0; j < Object.keys(pnls).length; j++) {
      if (pnls[i].symbol === margins[j].symbol) {
        console.log('confirm symbol')
        console.log('confirm symbol pnl', pnls[i].symbol)
        console.log('confirm symbol margins', margins[j].symbol)

        if (pnls[i].pnl > margins[j].margin * 0.7) {
          console.log('here pnl[i]')
          console.log('margin[j]', margins[j])
          const x = await SCMP.scmpSellALL(pnls[i].symbol)
          const buyit = {
            text: 'alert',
            msg: `⚠️ แจ้งเตือน\n                     เหรียญ : ${
              pnls[i].symbol
        ...
        `
          ...
          await lineNotifyPost.postLineNotify(buyit)
        } else if (pnls[i].pnl > margins[j].margin * 0.5) {
          const buyit = {
            text: 'alert',
            msg: `⚠️ แจ้งเตือน\n                     เหรียญ : ${
              pnls[i].symbol
        ...
        `
          ...
          await lineNotifyPost.postLineNotify(buyit)
        } else if (pnls[i].pnl > margins[j].margin * 0.3) {
          const buyit = {
            text: 'alert',
            msg: `⚠️ แจ้งเตือน\n                     เหรียญ : ${
              pnls[i].symbol
