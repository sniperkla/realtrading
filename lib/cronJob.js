...
const check50Percent = async (apiKey, secretKey) => {
  ...
  const task1 = marginStartday?.value - walletBalance
  const daily = (task1 / marginStartday?.value) * 100
...
const everyStartDay = async (apiKey, secretKey) => {
  ...
  const task1 = check?.value - walletBalance
  const daily = (task1 / check?.value) * 100
