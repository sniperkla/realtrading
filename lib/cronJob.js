...
const tpPayload = async (status, symbol) => {
  ...
  const sumCountWinRate = winrate[0].win + winrate[0].lose

  const percentWin = (winrate[0]?.win / sumCountWinRate) * 100
  const percentLoss = (winrate[0]?.lose / sumCountWinRate) * 100
