...
const checkTakeProfit4Step = async (initMargin) => {
  ...
  if (log.length !== 0) {
    ...
    for (let i = 0; i < symbol.length; i++) {
      ...
      if (unPNL[0].unRealizedProfit !== 0) {
        update.$setOnInsert = { staticUnrealizePnL: unPNL[0].unRealizedProfit }
      }
      try {
        await Lastestpnl.updateOne({ symbol }, update, { upsert: true })
      } catch (error) {
        console.error('Error updating Lastestpnl:', error)
      }
...
const tpPayload = async (status, symbol) => {
  ...
  const message =
    `💵 เหรียญ : ${symbol} ${
      status === 'WIN'
        ? `Takeprofit ไปแล้ว : ${lastestPnl.unrealizePnL}`
        : status === 'LOSE'
        ? `StopLoss ไปแล้ว : ${lastestPnl.unrealizePnL}`
        : `Liquildation ไปแล้ว : ${lastestPnl.unrealizePnL}`
    }` + `${message2} 💵`
