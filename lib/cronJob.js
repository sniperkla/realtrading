...
const checkStopLoss4Step = async () => {
  ...
  for (let i = 0; i < symbol.length; i++) {
    ...
    if (side === 'BUY') {
      if (getPrice <= checkTakeprofitZone?.zone3) {
        name = 'zone75'
        ...
        await Log.findOneAndUpdate(
          { symbol: symbol[i] },
          { lockStopLoss: { zone3: true, zone2: false, zone1: false } }
        )
        ...
        await Log.findOneAndUpdate(
          { symbol: symbol[i] },
          { lockStopLoss: { zone2: true, zone1: false } }
        )
        await updateStopLoss(
          symbol[i],
          stopPriceCal,
      ...
      if (getPrice >= checkTakeprofitZone?.zone3) {
        ...
        await Log.findOneAndUpdate(
          { symbol: symbol[i] },
          { lockStopLoss: { zone3: true, zone1: false, zone2: false } }
        )
        await updateStopLoss(
          symbol[i],
          stopPriceCal,
...
const checkStatusZone = async () => {
  ...
  const zone1 = alls.filter((item) => {
    return item.zone1 === true
  })
  const zone2 = alls.filter((item) => {
    return item.zone2 === true
  })
  ...
  const buyit = {
    text: 'updatestoploss',
    msg: `SUMMARY  zone25: ${zone1.length} / zone50: ${
      zone2?.length
    } / zone75: ${zone3?.length || 0}`
  }
  await lineNotifyPost.postLineNotify(buyit)
}
