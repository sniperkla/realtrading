...
const check50Percent = async (apiKey, secretKey) => {
  ...
  const daily =
    ((marginStartday?.value - walletBalance) / marginStartday?.value) * 100 || 0
  console.log('ddd', walletBalance)
  await storeHighest(value.highest, daily)
  return 0 // disable
...
module.exports = { checkTakeProfit4Step, check50Percent, everyStartDay }
...
const tpPayload = async (status, symbol) => {
  const message2 = await payload.payloadPnl(symbol)
  const message =
    `💵 เหรียญ : ${symbol} ${
      status === 'WIN'
        ? 'Takeprofit ไปแล้ว'
...
const storeHighest = async (highest, daily) => {
