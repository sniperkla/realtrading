...
const check50Percent = async (apiKey, secretKey) => {
  ...
  if (daily > marginStartday?.margin) {
    await storeHighest('daily', daily)
  }
  if (marginBalance > marginStartday?.peakvalue) {
    await InitMarginMonthly.updateOne(
      { _id: 'marginstartday' },
      { peakvalue: marginBalance }
    )
  }
  const initmarginMonthly = await InitMarginMonthly.findOne({
    _id: 'marginstartday'
  })
  task2 = initmarginMonthly?.peakvalue - marginBalance
  cum = (task2 / initmarginMonthly?.peakvalue) * 100
  if (cum > initmarginMonthly?.cum) {
    await storeHighest('cum', cum)
  }
  // }
  // return 0 // disable
...
const everyStartDay = async (apiKey, secretKey) => {
  ...
  task2 = check?.peakvalue - marginBalance
  cum = (task2 / check?.peakvalue) * 100
...
const every1hrPayload = async (type) => {
  await delay(71000)
  ...
  type !== 'bot' ? await checkEvery1hr.checkOrders(apiKey, secretKey) : null
...
const settingTask = async () => {
  ...
  if (checkSetting[0]?.status === true) {
    ...
    const compare =
      ...
      checkSetting[0]._id !== 'sellAll_when_toggle_Mbalnce2Level'
        ? getAccountInfo?.totalUnrealizedProfit || 0
        : getAccountInfo?.totalMarginBalance || 0
    if (
      checkSetting[0]?._id === 'sellAll_when_toggle_2Level' &&
      compare >= checkSetting[0]?.start
    ) {
    ...
    } else if (checkSetting[0]?._id === 'sellAll_when_toggle_martingale') {
      const martingale = await martinglale?.find()
      const logs = await Log.find()
      const list =
        checkSetting[0].type === 'opened'
    ...
    if (checkSetting[0]?._id === 'sellAll_when_toggle_2Level') {
      ...
      if (compare <= checkSellAll?.end && checkSellAll.executed) {
        const init = {
          text: 'initsmcp',
        ...
        await setting.findOneAndUpdate(
          { _id: 'sellAll_when_toggle_2Level' },
          { executed: false, start: 999999 }
        )
        await botCommand.closeAllMarket()
      }
    } else if (checkSetting[0]?._id === 'sellAll_when_toggle_Mbalnce2Level') {
      const checkSellAll = await setting.findOne({
        _id: 'sellAll_when_toggle_Mbalnce2Level'
      })
      if (compare <= checkSellAll?.end && checkSellAll.executed) {
        ...
        const buyit = {
          text: 'debug',
          msg: `ขายหมด on sellAll_when_toggle_Mbalnce2Level unPNL: ${compare} Value ที่ ตั้งค่า: Start ${checkSellAll?.start} End ${checkSellAll?.end}`
        }
        await lineNotifyPost.postLineNotify(buyit)
        await setting.findOneAndUpdate(
          { _id: 'sellAll_when_toggle_Mbalnce2Level' },
          { executed: false, start: 999999, end: 0 }
        )
        await botCommand.closeAllMarket()
      }
      ...
      (checkSetting[0]?._id === 'sellAll_when_toggle_martingale' &&
        compare >= sumMartingale * checkSetting[0]?.multiply) ||
      2
    ) {
      const init = {
        text: 'initsmcp',
        msg: `⚠️ เข้าเงื่อนไข UnPNL :${compare} >= Martingale*${
          checkSetting[0]?.multiply || 2
        } : ${sumMartingale * (checkSetting[0]?.multiply || 2)} ⚠️`
      }
      await lineNotifyPost.postLineNotify(init)
      const buyit = {
        text: 'debug',
        msg: `ขายหมด on sellAll_when_toggle unPNL: ${compare} Value ที่ ตั้งค่า: ${
          sumMartingale * (checkSetting[0]?.multiply || 2)
        }`
      }
      await lineNotifyPost.postLineNotify(buyit)
      await botCommand.closeAllMarket()
...
const storeHighest = async (type, value) => {
  if (type === 'daily') {
    await InitMarginMonthly.findOneAndUpdate(
      {
        _id: 'marginstartday'
