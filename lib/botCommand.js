...
const adjustTp = async (symbol, tp) => {
  ...
  console.log('status bi', binaceTakeProfitManual.status)
  if (binaceTakeProfitManual.status === 200) {
    console.log('suceess fully')
    ...
    console.log('canclejajajaajaj', cancle)

    await Log.findOneAndUpdate(
      { symbol: symbol },
      { binanceTakeProfit: binaceTakeProfitManual }
    )
    const buyit = {
      text: 'initsmcp',
