...
const resetMartingaleWithValue = async (symbol, value) => {
  ...
  const stackLose = stackLoseMap.filter((item, index) => {
    if (value >= item.margin) return item.stackLose[index]
  })

  console.log('test stacklose', stackLose)

  const setMar = await Martinglale.findOneAndUpdate(
    { symbol: symbol },
    { $set: { previousMargin: value, stackLose: stackLose } }
  )
  if (setMar) {
    const buyit = {
      text: 'initsmcp',
