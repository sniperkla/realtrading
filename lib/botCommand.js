...
const sellAllWhenToggleMbalance = async (value) => {
  try {
    ...
    const msg = {
      text: 'initsmcp',
      msg: `↪️ ตั้งค่า เมื่อ กำไรทิพย์ ถึง MarginBalance : ${value} สั่งขายทั้งหมด สำเร็จ สถานะ : ✅ ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(msg)
  } catch (error) {
    console.log('error from botcommand : ', error)
...
const sellAllWhenToggleMbalance2Level = async (start, end) => {
  try {
    ...
    const msg = {
      text: 'initsmcp',
      msg: `↪️ ตั้งค่า เมื่อ กำไรทิพย์ ถึง MarginBalance start: ${start} end:${end} สั่งขายทั้งหมด สำเร็จ สถานะ : ✅ ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(msg)
  } catch (error) {
    console.log('error from botcommand : ', error)
