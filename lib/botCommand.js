...
const adjustTp = async (symbol, tp) => {
  ...
  console.log('this is ', sideMarket?.binanceTakeProfit?.side)

  const binaneTakeProjitManual = await apiBinance.manualTakeProfit(
    symbol,
  ...
  if (binaneTakeProjitManual.status === 200) {
    await Log.findOneAndUpdate(
      { symbol: symbol },
      { binanceTakeProfit: binaneTakeProjitManual }
    )
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ เลื่อน TakeProfit to ${parseFloat(tp).toFixed(
        4
      )} สำเร็จ ${symbol}\n                     ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(buyit)
...
module.exports = { buyed, resetMartingale, resetMartingaleWithValue, adjustTp }
