...
const sellAllWhenToggleMartingale = async (multiply, type) => {
  try {
    const checkAnotherSettings = await setting?.find()
    const checkAnotherSetting = checkAnotherSettings.filter((item) => {
      return item._id !== 'sellAll_when_toggle_martingale'
    })
    const checkSetting = await setting?.findOne({
      _id: 'sellAll_when_toggle_martingale'
    })
    ...
    if (checkSetting) {
      await setting.findOneAndUpdate(
        { _id: 'sellAll_when_toggle_martingale' },
        { status: true, multiply: multiply, type: type }
      )
    }
    if (!checkSetting) {
      await setting.create({
    ...
    const msg = {
      text: 'initsmcp',
      msg: `↪️ ตั้งค่า เมื่อ UnPNL ถึง Martingale * ${multiply} type : ${type} value : ${
        sumMartingale * multiply
      } สั่งขายทั้งหมด สำเร็จ สถานะ : ✅ ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(msg)
  } catch (error) {
    console.log('error from botcommand : ', error)
...
const settingStatusWhenMartingale = async (status) => {
  const checkSetting = await setting?.findOne({
    _id: 'sellAll_when_toggle_martingale'
  })
  const checkAnotherSettings = await setting?.find()
  ...
  if (checkSetting) {
    ...
    const msg = {
      text: 'initsmcp',
      msg: `↪️ ตั้งค่า เงื่อนไขเมื่อ unPNL ถึง : Martingale*2 สั่งขายทั้งหมด สถานะ : ${
        status === 'active' ? '✅' : '❌'
      }  ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(msg)
  ...
  if (!checkSetting) {
    const msg = {
      text: 'initsmcp',
      msg: `⚠️ ไม่พบการตั้งค่า เงื่อนไขเมื่อถึง Martingale*2 ที่กำหนด ⚠️`
    }
    await lineNotifyPost.postLineNotify(msg)
  }
}
...
module.exports = {
  ...
  sellAllWhenToggleMartingale,
  settingStatusWhenMartingale,
  showSetting,
  showAll
}
