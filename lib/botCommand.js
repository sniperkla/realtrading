...
const adjustSl = async (symbol, sl, type) => {
  ...
  console.log('urhere this is type session', type)
  ...
  if (binanceStopLossManual.status === 200) {
    ...
    if (type) {
      await Log.updateOne(
        { symbol: symbol },
        { $set: { binanceStopLoss: binanceStopLossManual.data, type: type } }
      )
    } else if (!type) {
      await Log.updateOne(
        { symbol: symbol },
...
const mocklog = async (symbol, market, btp, smcp) => {
  ...
  if (!checkLog) {
    ...
    if (market === '0') {
      ...
      const buyit = {
        text: 'initsmcp',
        msg: `↪️ Mock ไม่สำเร็จ มีไม้เปิดอยู่ ${symbol}\n                     ByOwner ↩️`
      }
    }
  }
  await lineNotifyPost.postLineNotify(buyit)
...
const delMockLog = async (symbol) => {
  ...
  if (checkLog) {
    ...
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ ลบ Mock ไม่สำเร็จ ไม่มีไม้เปิดอยู่ ${symbol}\n                     ByOwner ↩️`
    }
  }
  await lineNotifyPost.postLineNotify(buyit)
}
