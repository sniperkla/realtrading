...
const mocklog = async (symbol, market, btp, smcp) => {
  ...
  if (!checkLog) {
    if (market === '1') {
      ...
      await Pearson.create({ symbol: symbol, BTP: btp })
      buyit = {
        text: 'initsmcp',
...
const unsetMartingale = async (symbol) => {
  const unsetMar = await Martinglale.findOneAndUpdate(
    { symbol: symbol },
    { $set: { active: false } },
    { upsert: true }
  )
  if (unsetMar) {
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ Unset Martingale สำเร็จ ${symbol}\n                     ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(buyit)
  }
}
const setMartingale = async (symbol) => {
  const setMar = await Martinglale.findOneAndUpdate(
    { symbol: symbol },
    { $set: { active: true } },
    { upsert: true }
  ...
  if (setMar) {
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ Set Martingale สำเร็จ ${symbol}\n                     ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(buyit)
  }
}
const unSetMartingaleAll = async () => {
  console.log('u here unset mar')
  const unSetMarAll = await Martinglale.updateMany({ $set: { active: false } })

  if (unSetMarAll) {
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ UnSet Martingale ทั้งหมดสำเร็จ \n                     ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(buyit)
  }
}
const setMartingaleAll = async () => {
  console.log('u here set mar')
  const setMarAll = await Martinglale.updateMany({ $set: { active: true } })

  if (setMarAll) {
    const buyit = {
      text: 'initsmcp',
      msg: `↪️ Set Martingale ทั้งหมดสำเร็จ \n                     ByOwner ↩️`
    }
    await lineNotifyPost.postLineNotify(buyit)
  }
}
