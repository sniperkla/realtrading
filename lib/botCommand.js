...
const setMarginStart = async (value) => {
  const checkInitMargin = await InitMargin.find({ _id: 'initmargin' })

  if (checkInitMargin) {
    await InitMargin.updateOne({ _id: 'initmargin' }, { value: value })
    const msg = {
      text: 'initsmcp',
...
const setMarginStartMonth = async (value) => {
  const checkInitMargin = await InitMargin.find({ _id: 'initmarginPort' })
  if (checkInitMargin) {
    await InitMargin.updateOne({ _id: 'initmarginPort' }, { value: value })
    const msg = {
      text: 'initsmcp',
