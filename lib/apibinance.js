...
const getOrder = async (orderId, symbol, apiKey, secretKey) => {
  ...
  try {
    ...
    console.error(
      `Error sending Binance API request getOrder: ${symbol}`,
      error.response.data
    )
    return error.response
  }
...
const getPnl = async (symbol, orderId, apiKey, secretKey) => {
  const timestamp = Date.now()
  ...
  const url = `${api_url}/income?${queryString}&signature=${signature}`
...
const getUserTrades = async (symbol, orderId, apiKey, secretKey) => {
  const timestamp = Date.now()
  ...
  const url = `${api_url}/userTrades?${queryString}&signature=${signature}`
