...
const postBinannce = async (
  ...
  console.log(symbol)
  console.log(side)
  console.log('price cal', priparseFloat(priceCal.toFixed(pricePrecision)))
  console.log('quantity', parseFloat(quantity.toFixed(qtyPrecision)))
  console.log('takeprofit', parseFloat(takeprofit.toFixed(pricePrecision)))
  console.log('stropLoss', parseFloat(stopPriceCal.toFixed(pricePrecision)))

  let params = {}
  if (type === 'BOS_MARKET_LIMIT') {
    params = {
      symbol: symbol,
      side: side,
      type: 'LIMIT',
  ...
  } else if (type === 'BOS_TAKE_PROFIT_LIMIT') {
    params = {
      symbol: symbol,
  ...
  } else if (type === 'BOS_STOP_LOSS_LIMIT') {
    params = {
      symbol: symbol,
      side: side === 'BUY' ? 'SELL' : 'BUY',
      type: 'STOP_MARKET',
...
const getAllOpenOrder = async (symbol, apiKey, secretKey) => {
  const timestamp = Date.now()
