...
const payloadPnl = async () => {
  ...
  const cum = marginStartday?.cum

  const forMaxDrawDown = await InitMarginMonthly.findOne({
    _id: 'marginstartday'
  })
  const maxDrawDownDaily = forMaxDrawDown?.highest || 0
  const maxDrawDownCum = forMaxDrawDown?.highestcum || 0
  ...
  ).toFixed(4)}\n 🚀 Margin Balance :\n ${parseFloat(margin).toFixed(
    4
  )}\n 🚀 Available Balance :\n ${parseFloat(availableBalance).toFixed(
    4
  )}\n 🚀 Wallet Balance :\n ${parseFloat(walletBalance).toFixed(
    4
  )}\n 🚀 Unrealized PNL :\n ${parseFloat(unPNL).toFixed(
    4
  )}\n 🚀 All Cumulative PNL :\n ${
    parseFloat(allCumulativePnl).toFixed(2) || 'error'
  }%\n 🚀 Cumulative PNL :\n ${
    parseFloat(cumulativePnl).toFixed(2) || 'error'
  }%\n 🚀 Winrate All :\n ${winrate[0]?.win || 0} || ${parseFloat(
    percentWin
  ).toFixed(4)}%\n 🚀 LossRate All :\n ${winrate[0]?.lose || 0} || ${parseFloat(
    percentLoss
  ).toFixed(4)}%\n 🚀 Winrate Monthly :\n ${
    winrateMonthly?.win || 0
  } || ${parseFloat(percentWinMonthly).toFixed(2)}%\n 🚀 LossRate Monthy :\n ${
    winrateMonthly?.lose || 0
  } || ${parseFloat(percentLoseMonthly).toFixed(
    2
  )}%\n 🚀 Margin Balance Start Day :\n${
    parseFloat(marginStartday?.value).toFixed(2) || 'error'
  }\n 🚀 DD :\n -${
    parseFloat(daily).toFixed(2) || 'error'
  }% \n 🚀 Max DD :\n -${
    parseFloat(maxDrawDownDaily).toFixed(2) || 'error'
  }% \n 🚀 CUMD :\n -${
    parseFloat(cum).toFixed(2) || 'error'
  }% \n 🚀 MAX CUMDD :\n -${parseFloat(maxDrawDownCum).toFixed(2) || 'error'}%`
  return message
}
