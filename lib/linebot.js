...
const messageReply = async (body) => {
  ...
  if (event === 'postback') {
    ...
    const check = await SMCP.scmpSellALL(data, apiKey, secretKey)
    if (check.status === true) {
      await martingaleUpdate.update(data, 'LOSE', margin)
      await Payload.payloadPnl()
    }
  }
...
const sendReply = async (text, replyTokens, replyedMsg) => {
  ...
  for (i = 0; i < Object.keys(symbol).length; i++) {
    if (replyedMsg === 'list' || replyedMsg === '9') {
      ...
      const messages = await checkMsg(symbol[i])
      const messsagex = { type: 'text', text: messages }
      // messagesx.push({
      //   type: 'text',
      //   text: messages[i]
      // })

      await selfReply(replyTokens, messsagex)
    }
  }
...
const selfReply = async (replyTokens, message) => {
  console.log('hello ur here selfReply', message)
  try {
    const response = await axios.post(
      LINE_MESSAGING_API,
      {
        replyToken: replyTokens, // Can be a single string or array of strings
        messages: [message] //[flexMessage]
      },
      {
        headers
      }
