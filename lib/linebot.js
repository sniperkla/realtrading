...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const commandList = [
      ...
      {
        cmd: '☠️ จัดการเหรียญ ☠️',
        example:
          '✅ filter_sym symbol:ABCUSDT.P status:true \n ❌ filter_sym symbol:ABCUSDT.P status:false'
      },
      {
        cmd: '☠️ แสดงเหรียญใช้งาน ☠️',
        example: 'show_sym'
      },
      {
        cmd: '☠️ ตั้งค่า Margin เริ่มต้น ☠️',
        example: 'set_marginStart value :100'
      },
      {
        cmd: '☠️ ตั้งค่า Margin เริ่มต้น เดือนใหม่ ☠️',
        example: 'set_marginStartMonth value :100'
      }
    ]
    ...
    await sendReply(symbol, replyToken, replyedMsg)
  } else if (replyedMsg.includes('set_marginStart')) {
    setMarginStart = replyedMsg.match(/value:(\S+)/)[1]
    await botCommand.setMarginStart(setMarginStart)
  } else if (replyedMsg.includes('set_marginStartMonth')) {
    setMarginStart = replyedMsg.match(/value:(\S+)/)[1]
