...
const sendReply = async (text, replyTokens, replyedMsg) => {
  ...
  if (replyedMsg === 'auto') {
    const checkAutoTakeProfit = await setting.find()
    if (checkAutoTakeProfit.length === 0) {
      await setting.create({ autoTakeprofit: false })
    }
    const autoTakeprofits = await setting.find()

    if (autoTakeprofits[0].autoTakeprofit === true) {
      await setting.UpdateOne({ autoTakeprofit: false })
    } else if (autoTakeprofits[0].autoTakeprofit === false) {
      console.log('u should be here')
      await setting.UpdateOne({ autoTakeprofit: true })
    }
  }
