...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const settingCommandList = [
      ...
      {
        cmd: 'เปิดใช้งาน unPNL เมื่อ Martingale * multiply สั่งขายทั้งหมด',
        example:
          '😎 toggle_martingale_setting_sell multiply:2 type:all  // ทำงานอัตโนมัติ \n toggle_martingale_setting_sell multiply:2 type:opened  // ทำงานอัตโนมัติ \n ✅ toggle_martingale_setting_status status:active เปิด \n ❌ toggle_martingale_setting_status status:deactive ปิด'
      }
    ]
    ...
    const symbolArray = symbol.split(',').toUpperCase().replace(/\.P$/, '')
  ...
  } else if (replyedMsg.includes('toggle_martingale_setting_sell')) {
    const multiply = replyedMsg?.match(/muitiply:(\S+)/)[1]
    const type = replyedMsg?.match(/type:(\S+)/)[1]

    await botCommand.sellAllWhenToggleMartingale(multiply, type)
  } else if (replyedMsg.includes('toggle_martingale_setting_status')) {
    const status = replyedMsg?.match(/status:(\S+)/)[1]
    await botCommand.settingStatusWhenMartingale(status)
