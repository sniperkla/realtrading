...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const settingCommandList = [
      {
        cmd: 'เปิดใช้งาน กำไรทิพย์ ถึงที่กำหนด สั่งขายทั้งหมด',
        example: 'setting_sell_toggle value:1000 // ทำงานอัตโนมัติ'
      },
      {
        cmd: 'ตั้งค่า เปิด - ปิด กำไรทิพย์ ถึงที่กำหนด สั่งขายทั้งหมด',
        example:
          '✅ setting_status_toggle status:active เปิด  \n ❌ setting_status_toggle status:deactive ปิด'
      },
      {
        cmd: 'เปิดใช้งาน กำไรทิพย์ ถึง xxx เมื่อแตะ xxx สั่งขายทั้งหมด',
        example:
          'setting_sell_toggle_2level start:1000 end:1200 // ทำงานอัตโนมัติ'
      },
      {
        cmd: 'ตั้งค่า เปิด - ปิด กำไรทิพย์ ถึง xxx เมื่อแตะ xxx สั่งขายทั้งหมด',
        example:
          '✅ setting_status_toggle_2level status:active เปิด  \n ❌ setting_status_toggle_2level status:deactive ปิด'
      }
    ]
  ...
  } else if (replyedMsg.includes('setting_status_toggle_2level')) {
    const status = replyedMsg?.match(/status:(\S+)/)[1]
