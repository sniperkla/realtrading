...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    setSymbol = replyedMsg.match(/symbol:(.+)/)[1]
    setPriceCal = replyedMsg.match(/pricecal:(.+)/)[1]
    setStopLoss = replyedMsg.match(/sl:(.+)/)[1]
    setSide = replyedMsg.match(/side:(.+)/)[1]
    result = {
      version: 'v3.1',
      type: 'MARKET',
      symbol: setSymbol,
      priceCal: setPriceCal,
      stopPriceCal2: setStopLoss
    }
...
const sendReply = async (text, replyTokens, replyedMsg) => {
  ...
  console.log('flexpayload', flexPayload2)
