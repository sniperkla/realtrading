...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const commandList = [
      {
        cmd: 'เปิดคำสั่งซื้อ',
        example: 'set_market symbol:ABCUSDT pricecal:1111 sl:1111 side:buy'
      },
      {
        cmd: 'รีเซท martingale เริ่มต้น',
        example: 'reset_mar symbol:ABCUSDT'
      },
      {
        cmd: 'รีเซท martingale ด้วยค่า',
        example: 'reset_mar symbol:ABCUSDT value:10'
      },
      {
        cmd: 'เลื่อน takeprofit',
        example: 'set_tp symbol:ABCUSDT tp:13.052'
      },
      {
        cmd: 'เลื่อน stoploss',
        example: 'set_sl symbol:ABCUSDT sl:13.052'
      },
      {
        cmd: 'จำลองเปิด market',
        example: 'set_mock symbol:ABCUSDT btp:-1 smcp:1'
      },
      {
        cmd: 'ยกเลิกจำลอง market',
        example: 'rm_mock symbol:ABCUSDT'
      },
      {
        cmd: 'ปิดไม้ทั้งหมด',
    ...
    const messagesx = messagesx.push({
      type: 'text',
    ...
    const symbol = setSymbol.toUpperCase()
    await botCommand.adjustSl(symbol, setSl)
...
const aloneReply = async (messagex) => {
  try {
    const response = await axios.post(
      LINE_MESSAGING_API,
      {
