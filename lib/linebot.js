...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const commandList = [
      ...
      },
      {
        ...
        example: 'set_mock symbol:ABCUSDT.P btp:-1 smcp:1'
      },
      {
      ...
      {
        cmd: 'ยกเลิก Martingale แต่ละเหรียญ',
        example: 'unset_mar symbol:ABCUSDT.P'
      },
      {
        cmd: 'เซท Martingale แต่ละเหรียญ',
        example: 'set_mar symbol:ABCUSDT.P'
      },
      {
        cmd: 'ยกเลิก Martingale ทั้งหมด',
        example: 'unset_mar_all'
      },
      {
        cmd: 'เซท Martingale ท้งหมด',
        example: 'set_mar_all'
      }
    ]
    ...
    const setSymbol = replyedMsg.match(/symbol:(\S+)/)[1]
    const setBtp = replyedMsg.match(/btp:(\S+)/)[1]
    const setSmcp = replyedMsg.match(/smcp:(\S+)/)[1]

    const symbol = setSymbol.toUpperCase()
    await botCommand.mocklog(symbol, setMarket, setBtp, setSmcp)
  ...
  } else if (replyedMsg.includes('unset_mar')) {
    const symbol = replyedMsg
      .match(/symbol:(\S+)/)[1]
      .toUpperCase()
      .replace(/\.P$/, '')
    await botCommand.unsetMartingale(symbol)
  } else if (replyedMsg.includes('set_mar')) {
    const symbol = replyedMsg
      .match(/symbol:(\S+)/)[1]
      .toUpperCase()
      .replace(/\.P$/, '')
    await botCommand.setMartingale(symbol)
  } else if (replyedMsg.includes('set_mar_all')) {
    await botCommand.setMartingaleAll
  } else if (replyedMsg.includes('unset_mar_all')) {
    await botCommand.unSetMartingaleAll()
  } else if (
    replyedMsg === 'show' ||
    replyedMsg === 'list1' ||
    replyedMsg === 'list2' ||
