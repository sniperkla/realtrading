...
const sendReply = async (text, replyTokens, replyedMsg) => {
  ...
  let msg = []
  let x = ''
  if (Object.keys(symbol).length > 8) {
    for (let i = 0; i < Object.keys(symbol).length; i++) {
      if (replyedMsg === 'list' || replyedMsg === '9') {
        const unPNLs = await PNL.findOne({ symbol: symbol[i < 8 ? i : null] })
        msg.push(symbol[i < 8 ? i : null])
        flexPayload.push({
          title: symbol[i < 8 ? i : null],
          text: `PNL : ≈ ${
      ...
      } else if (replyedMsg === 'list2') {
        const unPNLs = await PNL.findOne({ symbol: symbol[8 + (i + 1)] })
        msg.push(symbol[8 + (i + 1)])
        flexPayload.push({
          title: symbol[8 + (i + 1)],
          text: `PNL : ≈ ${
            unPNLs.unrealizePnL > 0
