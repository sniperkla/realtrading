...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const setValue = replyedMsg?.match(/value:(\S+)/)[1] || null
    const symbol = setSymbol.toUpperCase()

    if (!setValue) await botCommand.resetMartingale(symbol)
    else await botCommand.resetMartingaleWithValue(symbol, setValue)
  } else if (replyedMsg.includes('set_tp')) {
    const setSymbol = replyedMsg.match(/symbol:(\S+)/)[1]
    const setTp = replyedMsg.match(/tp:(\S+)/)[1]

    const symbol = setSymbol.toUpperCase()
    if (setValue) await botCommand.resetMartingaleWithValue(symbol, setValue)
    else await botCommand.adjustTp()
  } else {
