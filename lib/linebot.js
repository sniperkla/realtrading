...
const messageReply = async (body) => {
  ...
  if (body.events[0].message.text === 'show') {
...
const sendReply = async (text, replyTokens) => {
  ...
  try {
    ...
    const unPNL = getAccountInfo?.totalUnrealizedProfit.toFixed(2) || 'error'
    const margin = getAccountInfo?.totalMarginBalance.toFixed(2) || 'error'
    messagesx.push({
      type: 'text',
      text: `สรุป                     \n                     ✅ กำไรทิพย์ : ${unPNL} $\n                     ✅ เงินคงเหลือ : ${margin} $ `
    })
    const response = await axios.post(
      LINE_MESSAGING_API,
      {
        ...
        messages: messagesx //[flexMessage]
      },
      {
        headers
      }
