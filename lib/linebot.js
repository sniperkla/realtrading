...
const messageReply = async (body) => {
  ...
  if (replyedMsg === 'mar' || replyedMsg === '1') {
    ...
    const commandList = [
      ...
      {
        cmd: 'ตั้งค่า กำไรทิพย์ ถึงที่กำหนด สั่งขายทั้งหมด',
        example:
          'set_sellall_toggle value:1000 status:active \n ❌ ปิดการใช้งาน set_sellall_toggle status:deactive'
      }
    ]
  ...
  } else if (replyedMsg.includes('set_sellall_toggle')) {
    const checkSetting = await setting.findOne({ _id: 'sellAll_when_toggle' })
    const value = replyedMsg?.match(/value:(\S+)/)[1]

    if (value) {
      console.log('should have value', value)

      await botCommand.sellAllWhenToggle(value, status)
    } else {
      console.log('should no value', value)

      await botCommand.sellAllWhenToggle(checkSetting.value || 0, status)
    }
