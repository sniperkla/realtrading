...
const LongTP4 = async (body, qtyPrecision) => {
  ...
  await takeprofitZone(priceDistanceLong, priceCal, symbol)
...
const ShortTP4 = async (body, qtyPrecision) => {
  ...
  const takeprofit4 = priceCal + TP4L
  await takeprofitZone(priceDistanceShort, priceCal, symbol)
...
const takeprofitZone = async (priceDistance, priceCal, symbol) => {
  const TP4 = priceDistance * 0.25
  const takeprofit4 = priceCal - TP4
  const TP3 = priceDistance * 0.5
  const takeprofit3 = priceCal - TP3
  const TP2 = priceDistance * 0.75
  const takeprofit2 = priceCal - TP2
