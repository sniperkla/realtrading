...
function calPercentZone(line, priceCal, stopPriceCal, takeprofit, list) {
  ...
  return {
    z1: calSetLine(line, s100distance, start, list),
    z2: calSetLine(2, h100distance, l2, list)
  }
}

function calSetLine(section, distance, start, list) {
  const l = distance / section
  const set = []
  for (i = 1; i <= section; i++) {
    set.push({ tp: start + i * l, qty: list[i - 1] })
  }
  console.log('calsetline', set)
  return set
}
